...
parser.add_argument('--ratio', type=float, default=0.1, help='the ratio of the training set')
...
def train_val_test_set(num_gene,label_file,Gene_file,TF_file,all_set_file,train_set_file,val_set_file,test_set_file,density,p_val=args.p_val):
    ...
    for pos in all_set:
        ...
        adj[target, tf] = 1.0
    coo = adj.tocoo()
    df = pd.DataFrame({'row': coo.row, 'col': coo.col})
    #f=pd.DataFrame(adj)
    df.to_csv('A_adj.csv', index=False)
    train_pos = {}
    val_pos = {}
    test_pos = {}
...
if __name__ == '__main__':
    data_type = args.data
    net_type = args.net
    exp=os.getcwd() + '/' + net_type + ' Dataset/' + data_type + '/TFs+' + str(args.num) + '/BL--ExpressionData.csv'
    expression=pd.read_csv(exp)
    expression=expression.shape[0]
    ...
    all_set_file = os.getcwd() + '\\' + net_type + '\\' + data_type + ' ' + str(args.num) + '/adj.csv'
    ...
    if net_type == 'Specific':
        ...
        train_val_test_set(expression,label_file, Gene2file, TF2file,all_set_file, train_set_file, val_set_file, test_set_file, density)
